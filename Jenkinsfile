...
pipeline {
    ...
    environment {
        ...
        strDockerImage ="yoonpapa/cicd_guestbook:${strDockerTag}"
    }

    stages {
        stage('Checkout') {
            steps {
                git branch: 'master', url:'https://github.com/JINYONG-LEE/guestbook.git'
            }
        }
        stage('Build') {
            steps {
        ...
        stage('Docker Image Push') {
            steps {
                script {
                    docker.withRegistry('', 'DockerHub_yoonpapa') {
                        oDockImage.push()
                    }
                }
            }
